...
/**
* Enhanced main application window with full-screen support and animations
...
public class MainFrame extends JFrame implements MainFrameInterface {
 ...
 private final Color DARK_BG = LoginFrame.DARK_BG;
 private final Color ACCENT_PINK = LoginFrame.ACCENT_PINK;
 private final Color ACCENT_BLUE = LoginFrame.ACCENT_BLUE;
 private final Color FIELD_BG = LoginFrame.FIELD_BG;
 private final Color TEXT_COLOR = LoginFrame.TEXT_COLOR;
 private final Color SIDEBAR_COLOR = LoginFrame.SIDEBAR_COLOR;
 ...
 public MainFrame() {
     setTitle("CNSC Motorpool - Management System");
 ...
 private void setupAccessControl() {
   ...
   Component[] menuComponents = menuBar.getComponents();
       ...
       if (comp instanceof JMenu) {
           ...
           if (menu.getText().equals("View")) {
               for (int i = 0; i < menu.getItemCount(); i++) {
                   ...
                   if (item != null && item.getText().equals("Users")) {
   ...
   // Find and disable user management button in sidebar if not admin
   ...
   for (Component comp : sidebarComponents) {
       if (comp instanceof JPanel && ((JPanel) comp).getLayout() instanceof BoxLayout) {
           ...
           for (Component buttonComp : buttons) {
               if (buttonComp instanceof JButton) {
                   JButton button = (JButton) buttonComp;
                   if (button.getText() != null && button.getText().equals("Users")) {
 ...
 // Add this class to the MainFrame class
 private static class Particle {
     ...
     public Particle(int startX, int startY, float size, float speed, float alpha) {
         this.x = startX;
         this.y = startY;
         this.speedY = speed;
         this.size = size;
 ...
 private static class ModernScrollBarUI extends javax.swing.plaf.basic.BasicScrollBarUI {
     private final Color thumbColorValue;
     
     public ModernScrollBarUI(Color thumbColor) {
         this.thumbColorValue = thumbColor;
     }
     
     @Override
     protected void configureScrollBarColors() {
         trackColor = new Color(40, 40, 60);
         // Don't reassign the final thumbColor field from BasicScrollBarUI
         // Instead, we'll use our own thumbColorValue in paintThumb
     }
     ...
     protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
         ...
         // Draw rounded thumb using our thumbColorValue instead of the inherited thumbColor
         g2d.setColor(thumbColorValue);
